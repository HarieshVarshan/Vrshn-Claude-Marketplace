#!/usr/bin/env python3
"""
Incremental PDF Indexing Script
Add individual PDF files to an existing index.
"""
...
def add_multiple_pdfs(
    ...
    """
    Add multiple PDFs to existing index.
    ...
    for path in pdf_paths:
        ...
        result = add_single_pdf(path, persist_dir, chunk_size, force)
        if result:
            stats["added"] += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add PDF files to existing vector index"
    )
    parser.add_argument(
        "pdfs",
        nargs="+",
        help="PDF file(s) to add"
    )
    parser.add_argument(
        "--db", "-d",
        default="./chroma_db",
